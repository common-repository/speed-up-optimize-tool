...
class Speed_Up_Optimize_Settings {
	...
	public function __construct( $parent ) {
		...
		// add_action( 'admin_menu', array( $this, 'add_menu_item' ) ); // @codingStandardsIgnoreLine
	...
	public function add_menu_item() {
		$page = add_options_page( __( 'App Perf Settings', 'speed-up-optimize' ), __( 'App Perf Settings', 'speed-up-optimize' ), 'manage_options', $this->parent->token . '_settings', array( $this, 'settings_page' ) );
	...
	public function settings_assets() {
		...
		// wp_register_script( $this->parent->token . '-settings-js', $this->parent->assets_url . 'js/settings' . $this->parent->script_suffix . '.js', array( 'farbtastic', 'jquery' ), '1.0.0' );
	...
	public function add_settings_link( $links ) {
		$settings_link = '<a href="options-general.php?page=' . $this->parent->token . '_settings">' . __( 'Settings', 'speed-up-optimize' ) . '</a>';
	...
	public function register_settings() {
		if ( is_array( $this->settings ) ) {
			...
			$nonce = sanitize_text_field( wp_create_nonce( 'appperf_nonce' ) );

			if ( isset( $_POST['tab'] ) ) {
				if ( wp_verify_nonce( $nonce, 'appperf_nonce' ) ) {
	...
	public function settings_page() {
		...
		$html .= '<h2>' . __( 'App Perf Settings', 'speed-up-optimize' ) . '</h2>' . "\n";

		$tab = '';
		// Proper nonce handling.
		if ( isset( $_GET['appperf_nonce'] ) ) {
			if ( wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['appperf_nonce'] ) ), 'appperf_nonce' ) ) {
				if ( isset( $_GET['tab'] ) && sanitize_text_field( wp_unslash( $_GET['tab'] ) ) ) {
